...
export let generator; // ссылка на меш генератора
export let generatorCollider; // его коллайдер

// Здоровье генератора
export let generatorHP = 10;
const maxGeneratorHP = 10;

export function setGeneratorHP(value) {
  generatorHP = Math.min(maxGeneratorHP, Math.max(0, value));
}
...
export function initScene(scene) {
  ...
  // Генератор (столб)
  const generatorGeo = new THREE.BoxGeometry(1, 8, 1);
  const generatorMat = new THREE.MeshStandardMaterial({ color: 0x00ccff, emissive: 0x004466 });
  generator = new THREE.Mesh(generatorGeo, generatorMat);
  generator.position.set(0, 4, 0); // центр высоты 8 -> y=4
  generator.castShadow = true;
  generator.receiveShadow = true;
  scene.add(generator);

  // Коллизионный меш генератора
  generatorCollider = new THREE.Mesh(generatorGeo, new THREE.MeshBasicMaterial({ visible: false }));
  generatorCollider.position.copy(generator.position);
  generatorCollider.visible = false;
  generatorCollider.userData.isGenerator = true;
  scene.add(generatorCollider);
  collisionObjects.push(generatorCollider);
